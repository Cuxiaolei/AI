...
def generate_single_yaml(output_dir, dataset_info, t_val, num_val, model):
    # 解析数据集配置
...
output:
  exp_name: {model}_{ds_name}_{t_val}_{num_val}
"""

    # 生成文件路径
    filename = f"{model}_{ds_name}_{t_val}_{num_val}.yaml"
    ...
    for ds in datasets:
        ...
        for t_val in ds["t_list"]:
            for num_val in num_vals:
                file_path = generate_single_yaml(output_dir, ds, t_val, num_val, model)
