...
def process_las_files_merge_scene(input_dir: str, output_dir: str, write_split: bool = True, train_ratio: float = 0.8) -> None:
    ...
    for idx, (scene_id, files) in enumerate(tqdm(scene_groups.items(), desc="Processing scenes")):
        ...
        # 在 np.save(coord.npy) 前添加
        coord_min = xyz.min(axis=0)
        coord_max = xyz.max(axis=0)
        coord_range = coord_max - coord_min
        print(f"[{scene_name}] 原始坐标范围：x={coord_min[0]:.2f}~{coord_max[0]:.2f} "
              f"y={coord_min[1]:.2f}~{coord_max[1]:.2f} z={coord_min[2]:.2f}~{coord_max[2]:.2f}")
...
if __name__ == "__main__":
    # === 按需修改你的输入/输出路径 ===
    INPUT_DIR = r"D:\user\Documents\ai\paper\1_process\dataSet\data_scannet_tower\input"
    OUTPUT_DIR = r"D:\user\Documents\ai\paper\1_process\dataSet\data_scannet_tower\output-b"
