...
# misc custom setting
batch_size = 7  # bs: total bs in all gpus
mix_prob = 0.8
empty_cache = True
...
model = dict(
    ...
    backbone=dict(
        ...
        point_grid_size=[
            [[3, 3, 3], [10, 10, 10], [1, 1, 5]],  # stage1：Encoder第一阶段
            [[3, 3, 3], [10, 10, 10], [1, 1, 5]],  # stage2：Encoder第二阶段
            [[3, 3, 3], [10, 10, 10], [1, 1, 5]],  # stage3：Encoder第三阶段
            [[3, 3, 3], [10, 10, 10], [1, 1, 5]]   # stage4：Encoder第四阶段
        ],
    ...
    criteria=[
        dict(type="CrossEntropyLoss", loss_weight=1.0, ignore_index=-1),  # 主分割损失
        dict(type="PLCCLoss", loss_weight=0.3, ignore_index=-1)           # PLCCL损失（权重0.3，可调试）
    ],
