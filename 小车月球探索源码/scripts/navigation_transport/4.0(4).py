...
class VoiceControlNavNode:
    def __init__(self, name):
        ...
        self.up_ramp_time = rospy.get_param('/up_ramp_time', 2.0)  # map节点名
    ...
    def words_callback(self, msg):
        ...
        if self.words is not None and self.words not in ['唤醒成功(wake-up-success)', '休眠(Sleep)',
                                                         '失败5次(Fail-5-times)',
                                                         '失败10次(Fail-10-times']:
            pass
    ...
    def control(self, x, y, w, set_status):
        ...
        if set_status == "detect":  # 检测
            ...
            rospy.ServiceProxy('/yolov5/stop', Trigger)()  # 关闭yolov5
            #rospy.sleep(1)
            # rospy.ServiceProxy('/yolov5/stop', Trigger)()
            #rospy.sleep(2)
            # self.play('4')
        elif set_status == 'pick1':  # 夹取
            rospy.ServiceProxy('/position_correction/start', Trigger)()  # 启动对齐
        ...
        while not rospy.is_shutdown() and self.running:
            ...
            if self.words is not None:
                if self.words == '开始执行任务':
                    ...
                    print("go")
                    self.move_base_status = 1
                    self.control(1.55, -0.43, -90, "detect")  # 检测进行
                    self.control(0.93, -3.17, 0, "pick1")  # 进行夹取
                    self.control(1.57, -0.04, 48, "place")  # 进行放置
                    self.control(0.93, -3.17, -185, "pick2")  # 进行夹取
                    self.control(1.57, -0.04, 49, "place")  # 进行放置
                    # rospy.ServiceProxy('/yolov5/stop', Trigger)()
                    ...
                    self.control(0.85, -0.01, -0.7, "back")  # 回到起始点
                    self.play('11')
                    print('>>>>>>>>>>>>>>>>>>>> 结束任务<<<<<<<<<<<<<<<<<')
                elif self.words == '休眠(Sleep)':
                    rospy.sleep(0.01)
