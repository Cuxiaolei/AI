        ...
        while not rospy.is_shutdown() and self.running:
            ...
            if self.words is not None:
                if self.words == '开始执行任务':
                    ...
                    self.control(0.98, -3.22, 1, "pick1")  # 进行夹取
                    self.control(1.57, -0.04, 42, "place")  # 进行放置
                    self.control(0.93, -3.22, -180, "pick2")  # 进行夹取
                    self.control(1.57, -0.04, 42, "place")  # 进行放置
                    # rospy.ServiceProxy('/yolov5/stop', Trigger)()
                    ...
                    self.control(1, -0.08, 10, "back")  # 回到起始点
                    self.play('11')
                    print('>>>>>>>>>>>>>>>>>>>> 结束任务<<<<<<<<<<<<<<<<<')
                elif self.words == '休眠(Sleep)':
                    rospy.sleep(0.01)
