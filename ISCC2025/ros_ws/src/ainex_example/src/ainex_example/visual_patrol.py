...
class VisualPatrol:
    def __init__(self, gait_manager):
        ...
        self.x_range = [0, 0.06]  # 前进脚步大小单位m
        self.yaw_range = [-15, 15]  # 转弯范围正负8度
        self.gait_manager = gait_manager
        # common.loginfo("self.gait_manager的值为：")
        # print("self.gait_manager的值为：",self.gait_manager)
        ...
        self.go_gait_param['z_swap_amplitude'] = 0.005
        self.go_dsp = [250, 0.22, 0.02]
        self.go_arm_swap = 20
        ...
        self.turn_gait_param['z_swap_amplitude'] = 0.005
        self.turn_dsp = [300, 0.22, 0.02]
        self.turn_arm_swap = 20
    ...
    def process(self, x, width):
        """
        根据目标物体的 x 坐标控制机器人转向和前进速度
        """


        center_x = (width / 2) + self.calib_config['center_x_offset']
        x_error = x - center_x  # 误差: 目标与中心的偏差

        if abs(x_error) < 10:
            # 误差非常小，不旋转
            yaw_output = 0
        elif abs(x_error) < width / 6:
            # 中等误差：先加最小旋转基础值，再按比例调整
            yaw_output = math.copysign(1, x_error) + misc.val_map(
                x_error,
                -width / 6, width / 6,
            ...
            # 大误差：直接取最大旋转值
            yaw_output = math.copysign(self.yaw_range[1], x_error)
        ...
        if abs(yaw_output) < 4:
            ...
            self.gait_manager.set_step(
                self.go_dsp, x_output, 0, int(-yaw_output),
                self.go_gait_param, arm_swap=self.go_arm_swap, step_num=0
            )
        else:
            # 大角度转向（以转向步态为主）
            self.gait_manager.set_step(
                self.turn_dsp, x_output, 0, int(-yaw_output),
