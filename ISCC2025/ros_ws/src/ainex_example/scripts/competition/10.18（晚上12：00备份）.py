...
class CompetitionNode(Common):
    ...
    descend_stairs_action_name = 'descend_stairs_fasts_1'
    crawl_left_action_name = 'crawl_left_6'  # 左行走动作名称
    crawl_right_action_name = 'crawl_right_7'  # 右行走动作名称
    place_block_action_name = 'place_block_3'  # 放置方块动作名称
    ...
    # 上台阶状态下的目标位置阈值
    enter_climb_stairs_y = 210 / 480  # 当检测到的标识像素坐标y值占图像的比例大于此值时进入此阶段
    ...
    crawl_left_y_stop = 205 / 480  # 左行走的y轴目标位置(范围可在ApproachObject里设置)时停止横向移动
    ...
    def exit_climb_stairs(self, stairs_data):
        ...
        if stairs_data is not None:
            ...
            # 记录动作组调用
            common.loginfo(f"调用动作组: {self.move_left}")
            ...
            if hasattr(self, 'missed_red_flag_count'):
                ...
                self.missed_red_flag_count = 1
            common.loginfo(f"未识别到红色标志，累计计数: {self.missed_red_flag_count}")

            # 超过阈值未识别红色标志，直接执行上台阶动作
            if self.missed_red_flag_count > 2:
                common.loginfo(f"超过阈值，直接执行上台阶动作")
                common.loginfo(f"调用动作组: {self.move_left}")
                self.motion_manager.run_action(self.move_left)
                common.loginfo(f"调用动作组: {self.move_left}")
                self.motion_manager.run_action(self.move_left)
    ...
    def exit_descend_stairs(self, stairs_data):
        ...
        if stairs_data is not None:
            ...
            if self.missed_red_flag_count > 2:
                ...
                common.loginfo(f"调用动作组: {self.move_left}")
                self.motion_manager.run_action(self.move_left)
                common.loginfo(f"调用动作组: {self.forward_step}")
    ...
    def exit_hurdles(self, hurdles_data):
        ...
        if hurdles_data is not None:
            ...
            if hasattr(self, 'missed_hurdles_flag_count'):
                ...
                self.missed_hurdles_flag_count = 1
            common.loginfo(f"未识别到跨栏标志，累计计数: {self.missed_hurdles_flag_count}")

            # 超过阈值直接执行跨栏动作
            if self.missed_hurdles_flag_count > 1:
                common.loginfo(f"超过阈值，直接执行跨栏动作")
                common.loginfo(f"调用动作组: {self.move_left}")
                self.motion_manager.run_action(self.move_left)
                common.loginfo(f"调用动作组: {self.hurdles_action_name}")
                self.motion_manager.run_action(self.hurdles_action_name)
    ...
    def enter_climb_stairs(self, stairs_data):
        ...
        if stairs_data is not None:
            ...
            if max(stairs_data.y, stairs_data.left_point[1],
                ...
                if self.count > 2:  # 主线程比较快，颜色检测回调慢一点，需要连续检测来排除滞后干扰
                    self.count = 0
                    self.gait_manager.disable()
    ...
    def exit_crawl_left(self, block_data):
        ...
        if block_data is not None:
            ...
            common.loginfo(f"左抓取位置偏差: {offset_x}，目标中心: {centerx_left}")
            ...
            common.loginfo(f"未检测到方块，向后微调，调用动作组: {self.back_step}")
            self.motion_manager.run_action(self.back_step)
            time.sleep(0.01)
    ...
    def exit_place_block(self, line_data):
        ...
        if line_data is not None:
            ...
            common.loginfo(f"state:exit_place_block, line_data.y: {line_data.y}")
            ...
            common.loginfo(f"调整步态参数完成，调用动作组: {self.move_left}")
            self.motion_manager.run_action(self.move_left)
            common.loginfo(f"调用动作组: {self.move_left}")
            self.motion_manager.run_action(self.move_left)

            self.gait_manager.set_step([320, 0.25, 0.02], 0.02, 0, 0, None, 0, 3)
            common.loginfo(f"调用动作组: {self.place_block_action_name}")
            ...
            if hasattr(self, 'missed_black_flag_count'):
                ...
                self.missed_black_flag_count = 1
            common.loginfo(f"未识别到黑色标志，累计计数: {self.missed_black_flag_count}")
