...
class CompetitionNode(Common):
    ...
    # 下台阶状态下的目标位置阈值
    enter_descend_stairs_y = 240 / 480  # 150
    descend_stairs_x_stop = 0.5
    descend_stairs_y_stop = 265 / 480


    # 向左抓取状态下的目标位置阈值
    enter_crawl_left_y = 170 / 480  # 当检测到的标识像素坐标y值占图像的比例大于此值时进入此阶段
    crawl_left_x_stop = 0.44  # 左行走的x轴目标位置范围可在ApproachObject里设置)时停止前后移动
    crawl_left_y_stop = 0.48  # 左行走的y轴目标位置(范围可在ApproachObject里设置)时停止横向移动

    # 向右抓取状态下的目标位置阈值，原理同上
    enter_crawl_right_y = 160 / 480  # 120;500
    crawl_right_x_stop = 0.64
    crawl_right_y_stop = 0.48

    # 放块状态下的目标位置阈值
    enter_place_block_y = 100 / 480
    place_block_x_stop = 320
    place_block_y_stop = 240
    ...
    def __init__(self, name):
        ...
        self.x_max = 0.033
        self.turn_x_max = 0.028


        # 减小巡线步幅，提高其他标志检测稳定性
        self.visual_patrol.update_go_gait(x_max=self.x_max)
        self.visual_patrol.update_turn_gait(x_max=self.turn_x_max)
    ...
    def enter_hurdles(self, hurdles_data):
        ...
        if hurdles_data is not None:
            print("state:enter_hurdles ", "hurdles_data.y:", hurdles_data.y, "self.enter_hurdles_y", self.enter_hurdles_y, "hurdles_data.height",hurdles_data.height)
    ...
    def enter_climb_stairs(self, stairs_data):
        ...
        if stairs_data is not None:
            print("state:enter_climb_stairs ", "stairs_data.y:", stairs_data.y, "enter_climb_stairs_y:",self.enter_climb_stairs_y, "stairs_data.height", stairs_data.height)
    ...
    def exit_climb_stairs(self, stairs_data):
        ...
        if stairs_data is not None:
            ...
            self.gait_manager.set_step([320, 0.25, 0.02], 0.015, 0, 0, None, 0, 1)
    ...
    def exit_descend_stairs(self, stairs_data):
        ...
        if stairs_data is not None:
            ...
            if self.missed_red_flag_count > 2:
                common.loginfo(f"超过阈值，直接执行下台阶动作")
                common.loginfo(f"调用动作组: {self.descend_stairs_action_name}")
    ...
    def exit_place_block(self, line_data):
        ...
        if line_data is not None:
            ...
            common.loginfo(f"state:exit_place_block, line_data.width: {line_data.width}, line_data.height: {line_data.height}")

            # 1. 计算目标位置（基于放块阈值参数）
            target_x = self.place_block_x_stop  # x轴目标位置
            target_y = self.place_block_y_stop  # y轴目标位置
            offset_x = line_data.x - target_x  # x轴偏差（当前x - 目标x）
            offset_y = line_data.y - target_y  # y轴偏差（当前y - 目标y）
            common.loginfo(f"放块位置偏差: x={offset_x:.1f}, y={offset_y:.1f}，目标x={target_x:.1f}, 目标y={target_y:.1f}"
                           f"当前目标x={line_data.x}, y={line_data.y}")
            # common.loginfo("line_data.x", line_data.x ,"line_data.y:",line_data.y)
            ...
            self.gait_manager.set_step([320, 0.25, 0.02], 0.018, 0, 0, None, 0, 4)  # 前进4步
