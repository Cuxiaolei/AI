...
class CompetitionNode(Common):
    ...
    crawl_left_action_name = 'crawl_left4'  # 左行走动作名称
    ...
    # 上台阶状态下的目标位置阈值
    enter_climb_stairs_y = 190 / 480  # 当检测到的标识像素坐标y值占图像的比例大于此值时进入此阶段
    climb_stairs_x_stop = 0.45  # 当检测到的标识像素坐标x值占图像的比例在此值附近(范围可在ApproachObject里设置)时停止前后移动
    climb_stairs_y_stop = 300 / 480  # 当检测到的标识像素坐标y值占图像的比例在此值附近(范围可在ApproachObject里设置)时停止横向移动
    climb_stairs_yaw_stop = 0  # 当检测到的标识角度在此值附近(范围可在ApproachObject里设置)时停止旋转移动
    # 下台阶状态下的目标位置阈值
    enter_descend_stairs_y = 1 / 480
    descend_stairs_x_stop = 0.5
    descend_stairs_y_stop = 180 / 480
    descend_stairs_yaw_stop = 0
    # 跨栏状态下的目标位置阈值
    enter_hurdles_y = 180 / 480
    hurdles_x_stop = 0.5
    hurdles_y_stop = 270 / 480
    hurdles_yaw_stop = 0
    # 向左抓取状态下的目标位置
    enter_crawl_left_y = 150 / 480  # 当检测到的标识像素坐标y值占图像的比例大于此值时进入此阶段
    crawl_left_x_stop = 280 / 640  # 左行走的x轴目标位置(范围可在ApproachObject里设置)时停止前后移动，这个控制左右距离
    crawl_left_y_stop = 60 / 480  # 左行走的y轴目标位置(范围可在ApproachObject里设置)时停止横向移动，这个控制前后距离
    crawl_left_yaw_stop = 0  # 当检测到的标识角度在此值附近(范围可在ApproachObject里设置)时停止旋转移动
    # 向右抓取状态下的目标位置阈值，原理同上
    enter_crawl_right_y = 100 / 480
    crawl_right_x_stop = 200 / 640
    crawl_right_y_stop = 100 / 480
    crawl_right_yaw_stop = 0
    # 放块状态下的目标位置阈值
    enter_place_block_y = 310 / 480
    place_block_x_stop = 200 / 640
    place_block_y_stop = 340 / 480
    ...
    def __init__(self, name):
        ...
        self.state = {'visual_patrol': [[500, 277],
                      ...
                      'crawl_right': [[500, 277],
                                      ['green', self.block_roi, self.image_process_size, self.set_block_color], False],
                      # 右抓取
                      'place_block': [[500, 277], ['black', self.intersection_roi, self.image_process_size,
        ...
        # 减小巡线步幅，提高其他标志检测稳定性
        self.visual_patrol.update_go_gait(x_max=0.023)
    ...
    def exit_climb_stairs(self, stairs_data):
        ...
        if stairs_data is not None:
            ...
            if self.approach_object.process(max(stairs_data.y, stairs_data.left_point[1], stairs_data.right_point[1]),
                                            stairs_data.x, stairs_data.angle,
                                            self.climb_stairs_y_stop * stairs_data.height,
                                            self.climb_stairs_x_stop * stairs_data.width, self.climb_stairs_yaw_stop,
                                            stairs_data.width, stairs_data.height):
    ...
    def exit_descend_stairs(self, stairs_data):
        ...
        if stairs_data is not None:
            self.missed_red_flag_count = 0
            # self.motion_manager.run_action(self.mcjks_action_name)
            print("state:exit_descend_stairs ", "stairs_data.y:", stairs_data.y)
            if self.approach_object.process(max(stairs_data.y, stairs_data.left_point[1], stairs_data.right_point[1]),
                                            stairs_data.x + self.calib_config['center_x_offset'], stairs_data.angle,
                                            self.descend_stairs_y_stop * stairs_data.height,
                                            self.descend_stairs_x_stop * stairs_data.width,
                                            self.descend_stairs_yaw_stop, stairs_data.width, stairs_data.height):
                self.gait_manager.disable()  # 关闭步态控制
    ...
    def exit_hurdles(self, hurdles_data):
        ...
        if hurdles_data is not None:
            ...
            if self.approach_object.process(
                    ...
                    hurdles_data.x + self.calib_config['center_x_offset'], hurdles_data.angle,
                    self.hurdles_y_stop * hurdles_data.height, self.hurdles_x_stop * hurdles_data.width,
                    self.hurdles_yaw_stop, hurdles_data.width, hurdles_data.height):
            ...
            if self.missed_hurdles_flag_count > 2:
                self.motion_manager.run_action(self.hurdles_action_name)
                self.missed_hurdles_flag_count = 0
                return True
    """
    ...
    def exit_hurdles(self, hurdles_data):
        ...
        if hurdles_data is not None:
            ...
            self.motion_manager.run_action(self.hurdles_action_name)
            rospy.sleep(0.5)
            self.visual_patrol.update_go_gait(x_max=0.025)
            self.visual_patrol.update_turn_gait(x_max=0.02)
            self.slow = True
            self.motion_manager.run_action('walk_ready')
            rospy.sleep(0.5)
        ...
        if stairs_data is not None:
            ...
            if max(stairs_data.y, stairs_data.left_point[1],
                ...
                if self.count > 3:  # 主线程比较快，颜色检测回调慢一点，需要连续检测来排除滞后干扰
                    self.count = 0
                    self.gait_manager.disable()
                    self.approach_object.update_approach_stop_value(20, 15, 6)  # 设置靠近目标停止的条件，分别为y, x, angle误差
        ...
        if stairs_data is not None:
            ...
            if max(stairs_data.y, stairs_data.left_point[1],
                ...
                if self.count > 1:
                    self.count = 0
                    self.gait_manager.disable()
                    self.approach_object.update_approach_stop_value(20, 15, 6)
                    self.motion_manager.run_action('hand_back')  # 手往后，防止遮挡
                    rospy.sleep(0.5)
                    return True
            else:
    ...
    def enter_crawl_left(self, block_data):
        if block_data is not None:
            ...
            if block_data.y > self.block_roi[0] * block_data.height and self.slow:
                self.visual_patrol.update_go_gait(x_max=0.015)   # 允许的最大速度
    ...
    def enter_place_block(self, line_data):
        if line_data is not None:
            ...
            print((line_data.right_point[0] - line_data.left_point[0]))
            if max(line_data.y, line_data.left_point[1],
                   line_data.right_point[1]) > self.enter_place_block_y * line_data.height and (
                    line_data.right_point[0] - line_data.left_point[0]) > 270:
                self.count += 1
                if self.count > 3:
                    self.count = 0
        ...
        return False
         ...
         #   self.no_line_count = getattr(self, 'no_line_count', 0) + 1
        #else:
         #   self.no_line_count = 0  # 识别到黑线，计数清零
    ...
    def enter_place_block(self, line_data):
        ...
        # 参数（可根据实际场景微调）
        positive_frames_threshold = 2  # 连续满足放置点条件才触发
    ...
    #     if block_data is not None:
    #         print("state:exit_crawl_left", " block_data.x:", block_data.x, " block_data.x:", block_data.y)
    #         centerx_left = self.crawl_left_x_stop * block_data.width
    #         if block_data.x - centerx_left > 20:
    #             self.motion_manager.run_action(self.move_right)
    #         elif block_data.x - centerx_left < -20:
    ...
    #
    #                 # 开始记录（仅第一次触发时初始化）
    #                 if not self.crawl_recording:
    #                     self.crawl_recording = True
    #                     self.crawl_record_start_time = time.time()
    #                     self.crawl_position_buffer = []  # 清空历史缓存
    #                     common.loginfo("开始记录物块位置（2秒）...")
    #
    #                 # 记录当前时间和坐标（每帧都记录，确保数据密度）
    #                 current_time = time.time()
    #                 if current_time - self.crawl_record_start_time < self.crawl_record_duration:
    #                     # 缓存坐标（x: 左右位置，y: 前后位置，可根据实际添加角度等）
    #                     self.crawl_position_buffer.append({
    #                         "x": block_data.x,
    #                         "y": block_data.y,
    #                         "width": block_data.width,
    #                         "height": block_data.height
    #                     })
    #                     return False  # 未完成记录，不退出状态
    #
    #                 # 3. 记录完成，计算平均坐标（减少噪声影响）
    #                 else:
    #                     self.crawl_recording = False
    #                     if not self.crawl_position_buffer:
    #                         common.logwarn("未记录到有效物块坐标，使用最后一次坐标")
    #                         avg_x = block_data.x
    #                         avg_y = block_data.y
    #                     else:
    #                         # 计算x和y的平均值
    #                         avg_x = sum([p["x"] for p in self.crawl_position_buffer]) / len(self.crawl_position_buffer)
    #                         avg_y = sum([p["y"] for p in self.crawl_position_buffer]) / len(self.crawl_position_buffer)
    #                     common.loginfo(f"位置记录完成，平均坐标: x={avg_x}, y={avg_y}")
    #
    #                     # 4. 调用动态夹取函数（传入平均坐标）
    #                     self.dynamic_crawl_left(avg_x, avg_y, block_data.width, block_data.height)
    #
    #                     # 5. 恢复巡线参数，退出夹取状态
    #                     self.visual_patrol.update_go_gait(arm_swap=0)
    #                     self.visual_patrol.update_turn_gait(arm_swap=0)
    #                     self.visual_patrol.update_go_gait(x_max=0.025)
    #                     self.visual_patrol.update_turn_gait(x_max=0.02)  # 转弯最大速度
    #                     self.slow = True
    #                     return True
    #
    #     else:
    ...
    def exit_crawl_right(self, block_data):
        if block_data is not None:
            print("block_data的信息：")
            print("state:exit_crawl_right", " block_data.x:", block_data.x, "block_data.y:", block_data.y)
            centerx_right = self.crawl_right_x_stop * block_data.width

            # 1. 原有左右位置调整逻辑（保持不变）
            if block_data.x - centerx_right > 20:
                self.motion_manager.run_action(self.move_right)
            elif block_data.x - centerx_right < -20:
                self.motion_manager.run_action(self.move_left)
            ...
            # 3. 位置达标：构造动态夹取参数，调用 run_dynamic_crawl_right（核心修改部分）
            elif block_data.y >= self.crawl_right_y_stop * block_data.height:
                time.sleep(0.5)
                # 二次确认位置（避免抖动误判）
                if block_data.y >= self.crawl_right_y_stop * block_data.height:
                    # -------------------------- 新增：构造动态夹取所需参数 --------------------------
                    # 从现有 block_data 提取参数，适配 run_dynamic_crawl_right 的输入格式
                    dynamic_block_data = {
                        "x": block_data.x,  # 物块中心点x坐标（样例：316）
                        "y": block_data.y,  # 物块中心点y坐标（样例：228）
                        "size": 2 * block_data.radius if hasattr(block_data, "radius") else 40,  # 物块尺寸（圆形用直径，默认40防错）
                        "img_width": block_data.width,  # 图像宽度（样例：640）
                        "img_height": block_data.height  # 图像高度（样例：480）
                    }
                    # 打印参数日志（调试用，可按需保留）
                    print(f"构造动态夹取参数：{dynamic_block_data}")

                    # -------------------------- 替换：调用动态夹取动作 --------------------------
                    # 替换原预设动作，执行动态生成的夹取动作
                    dynamic_action_result = self.motion_manager.run_dynamic_crawl_right(dynamic_block_data)
                    if not dynamic_action_result:
                        print("警告：动态夹取动作执行失败，可检查参数或硬件状态")
                    else:
                        print("动态夹取动作执行完成")
                    ...
                    self.visual_patrol.update_go_gait(x_max=0.02)
                    self.visual_patrol.update_turn_gait(x_max=0.03)
                    self.slow = True
                    self.delay_time = time.time() + 7.2  # 抓完色块延时后再识别横线
                    return True
    ...
    def exit_place_block(self, line_data):
        if line_data is not None:
            ...
            if self.approach_object.process(max(line_data.y, line_data.left_point[1], line_data.right_point[1]),
                                            line_data.x, line_data.angle,
                                            self.place_block_y_stop * line_data.height,
                                            self.place_block_x_stop * line_data.width, self.place_block_yaw_stop,
                                            line_data.width, line_data.height):
                self.gait_manager.disable()  # 关闭步态控制
    ...
    """
        if line_data is  None:
            # 机器人向前走五步，每步调用 forward_step，步间带短暂停顿
            for _ in range(3):
                self.motion_manager.run_action('forward_step')
                time.sleep(0.2)  # 适当延时保证动作
            self.gait_manager.disable()
            walking_param = self.gait_manager.get_gait_param()  # 获取当前的步态参数
            walking_param['body_height'] = 0.015  # 设置身体高度,单位米
            walking_param['pelvis_offset'] = 7  # 设置骨盆位置的前后偏移量,单位度
            ...
            common.loginfo('place_block')
            # 执行放置动作组
            self.motion_manager.run_action('place_block_action_name')            
            return True
        return False
    """
        ...
        while self.running:
            if self.start:
                ...
                for object_info in self.objects_info:
                    ...
                    # print(object_info)
                    # 调用机械臂动作，让手臂保持“手往后”姿态防止遮挡
                    # self.motion_manager.run_action('crawl_behind')
