...
class GaitManager:
    def __init__(self):
        ...
        # 步幅范围，单位m
        self.x_amplitude_range = [0.0, 0.04]
        self.y_amplitude_range = [0.0, 0.04]
        # 腿抬高范围，单位m 
        self.step_height_range = [0, 0.04]
        # 转弯范围，单位度
        self.rotation_angle_range = [0, 15]
        # 手臂摆动范围，单位度
    ...
    def get_gait_param(self):
        ...
        # common.loginfo(f"步态参数：init_roll_offset: {self.walking_param}")
    ...
    def update_pose(self, walking_param):
        ...
        # common.loginfo(f"步态参数：init_roll_offset: {self.walking_param.init_roll_offset}")
    ...
    def set_step(self, step_velocity, x_amplitude, y_amplitude, rotation_angle, walking_param=None, arm_swap=30, step_num=0):
        ...
        try:
            ...
            if step_num != 0:
                ...
                if self.state != 'walking' or not self.is_walking:
                    if step_num == 0:
                        self.state = 'walking'
                        res = rospy.ServiceProxy('/walking/get_param', GetWalkingParam)()
                        self.walking_param = res.parameters
